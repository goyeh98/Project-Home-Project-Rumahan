...
async function getGoogleFonts() {
  let response = await fetch(
    "https://webfonts.googleapis.com/v1/webfonts?sort=SORT_UNDEFINED&key=AIzaSyB69SADMp8KnBcOkoYAiIqnStqXj058R3M"
  );
  ...
  if (response.ok) {
    ...
    chrome.storage.sync.get(["standard"], function (result) {
      if (result["standard"]) {
        ...
        $(".font-preview-standard").css(
          "font-family",
          `"${standardFontData.family}", Arial`
        );
        $(
          '.fonts-list-standard option:contains("' +
            standardFontData.family +
            '")'
...
$(".load").on("click", () => {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    chrome.tabs.sendMessage(tabs[0].id, { type: "restoreImage" });
  });
});

// Added by Sailesh
$(".reduceColor").on("click", () => {
  chrome.storage.sync.set({ standard: false });
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    chrome.tabs.sendMessage(tabs[0].id, { type: "reduceColor" });
  });
});

// Added by Zico (Gif Stopper)
$(".stop").on("click", () => {
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    chrome.tabs.sendMessage(tabs[0].id, { type: "stopAnimation" });
  });
});

// Added by Zico (Text Editor)
$(".edit").on("click", () => {
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    chrome.tabs.sendMessage(tabs[0].id, { type: "editText" });
  });
});
...
$(".fonts-list-standard").change(function (event) {
  if (event.target.value !== "default") {
    ...
    $(".font-preview-standard").css(
      "font-family",
      `"${event.target.selectedOptions[0].label}", Arial`
    );
    chrome.runtime.sendMessage({
      type: "changeFont",
      last: lastSelectedStandard,
      fontStyle: "standard",
      family: event.target.selectedOptions[0].label,
      fontURL: event.target.value.replace("http", "https"),
    });
    chrome.storage.sync.set({
      standard: JSON.stringify({
        family: event.target.selectedOptions[0].label,
        url: event.target.value.replace("http", "https"),
      }),
    });
